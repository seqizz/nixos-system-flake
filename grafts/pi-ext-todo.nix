# Pi coding-agent extension: rpiv-todo, vendored as a self-contained store
# path so it runs both on the host AND inside llm-custody-pi without any
...
in
final.runCommandLocal "pi-ext-rpiv-todo-2.9.0" { } ''
  mkdir -p $out/node_modules/@juicesharp
  cp -R ${src}/packages/rpiv-todo/. $out/
